...
from .pipeline_nodes import LoggingNode, ProfanityFilterNode
...
class ChatConsumer(AsyncWebsocketConsumer): 
    ...
    async def receive(self, text_data):
        ...
        # --- PIPELINE START ---
        pipeline = Pipeline()
        pipeline.add_node(ProfanityFilterNode())
        pipeline.add_node(LoggingNode())
        ...
        message_data = await pipeline.run(message_data)
        # --- PIPELINE END ---

        # Save message to DB
        message_obj = await self.save_message(sender_user, message_data["message"])
